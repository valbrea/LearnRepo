...
Camp::Camp(const string camp_name, const int &camp_str, const int *str_per,
    ...
    : camp_name_(camp_name), number_(0), qty_(), camp_str_(camp_str),
      type_in_order_(), str_in_order_(), atk_in_order_(), index_(0) {
  time = 0;            // 两军共用一个时间
  min_str_ = camp_str; // 最小值先设定一个较高的数
  for (int i(0); i < 5; ++i) {
    type_in_order_[i] = type[seq[i]];
    str_in_order_[i] = str_per[seq[i]];
    if (str_in_order_[i] < min_str_)
      min_str_ = str_in_order_[i];
  }
}
// **************主函数**************
...
int main() {
  ...
  const string type[5] = {"dragon", "ninja", "iceman", "lion",
                          "wolf"};        //武士类型
