...
public:
  BigInt(const int *ptr, const int &digit, const int &negative); // 输出用这个
  BigInt(const string &str = ""); // 输入一般用这个
  ...
  // 这个函数用来扩展某个对象的空间至digit位，使其低位和对象s相同，高位如果不填s，
...
int main() {
  ...
  // ExtendFromLow()这些函数相当于已经在main函数前声明过了
  string a, b;
  char op;
  cin >> a;
  cin >> op;
...
BigInt::BigInt(const string &str) : ptr_(), digit_(0), negative_(-1) {
  ...
  digit_ = str.length();
  ptr_ = new int[digit_];
  // 因为输入正整数，所以不需要判断输入是否为0，negative_始终为-1
...
BigInt operator*(const BigInt &a, const BigInt &b) {
  ...
  for (int j(0); j < b.digit_; ++j) {   // b的每一位都要和a相乘
    for (int i(0); i < a.digit_; ++i) { // 每一轮加到a的最高有效位
      ...
      ptr_c[i + j] +=
          tmp_a.ptr_[i] * tmp_b.ptr_[j] + carry; // 这里要用+=，因为每一轮都会加
      if (ptr_c[i + j] > 9) {                    // 如果数字出现进位
        carry = ptr_c[i + j] / 10;               // 记录进位
        ptr_c[i + j] -= 10 * carry;              // 删掉高位
      } else {
        carry = 0;
      }
    }
    if (carry != 0) {               // 如果a的最高有效位还有进位
      ptr_c[a.digit_ + j] += carry; // 每j轮，c的最高位
    }
    ...
    while (divisor.digit_ >= b.digit_) {
      if (Compare(dividend, divisor) >= 0) {
        ...
        if ((pos + 1) < dividend.digit_) {
          char *temp_quotient = new char;
          sprintf(temp_quotient, "%d", quotient_top);
          quotient = quotient + BigInt(temp_quotient + string(divisor.digit_ - b.digit_, '0'));
