...
string Knight[5] = {"dragon", "ninja", "iceman", "lion",
                    "wolf"}; //武士名称，输出使用
...
class Weapon { //武器
private:
  int name, sword_attack,
      arrow_usetime; //武器名称，sword攻击力，arrow剩余使用次数
public:
  Weapon(int i, int satt)
      : name(i), sword_attack(satt), arrow_usetime(3) {} //武器状态初始化
...
class Warrior { //士兵
private:
  int Num, Life, Attack, Loyal, Step,
      l_life;           //编号，生命力，攻击力，忠诚度，步数
  ...
  Weapon *weapon[3];          //装备
public:
  Warrior(int n, Knight_name &name_, int &life, int &t_life,
          Command *COMD) { //初始化士兵属性
    Num = n;
    name = name_;
    Life = life;
    Attack = Attack_[name];
    ...
    case ninja:
      weapon[Num % 3] = new Weapon(Num % 3, int(Attack_[name] * 0.2));
      weapon[(Num + 1) % 3] =
          new Weapon((Num + 1) % 3, int(Attack_[name] * 0.2));
      break;
    default:
      break;
    };
...
class City {
private:
  int Number, Life, Flag,
      Flag_G[2]; //城市编号，生命元，旗帜，各方士兵连续胜利场次
...
public:
  ...
  void
  Minute_38() { //处理bomb事件，顺带处理35分钟时城市中两只warrior均被被射死的情况
    if (Warrior_In_City[0] and Warrior_In_City[1]) {
      ...
      if (!p and Warrior_In_City[0]->life() <= 0 and
          Warrior_In_City[1]->life() <= 0) {
        delete Warrior_In_City[0];
        Warrior_In_City[0] = NULL;
        delete Warrior_In_City[1];
        Warrior_In_City[1] = NULL;
...
class Command {
private:
  int Total_Life, Knights_Num, Colour,
      Enemy_Enter; //生命元，造士兵数量，command颜色，有多少敌人进入
...
public:
  ...
  void Minute_0() { //制造warrior及输出
    if (Total_Life >= Life_Make[Order[Colour][Knights_Num % 5]]) {
      ...
      Print_time();
      cout << Color[Colour] << ' ';
      Knight = new Warrior(Knights_Num + 1, Order[Colour][Knights_Num % 5],
                           Life_Make[Order[Colour][Knights_Num % 5]],
                           Total_Life, this);
      Knights_Num++;
    }
  }
...
void Game_Start() {
  ...
  for (int t = 0; t < times; ++t) {
    ...
    while (Hour * 60 <= Total_Minutes) {
      ...
      if (Hour * 60 + Minute > Total_Minutes)
        break;          //第35分钟
      else if (N > 1) { // city数大于1时才会发生arrow事件
        City *Cp[2] = {};
        Cp[0] = NULL;
        Cp[1] = &Citys[1];
      ...
      if (Hour * 60 + Minute > Total_Minutes)
        ...
        for (vector<City>::iterator i = Citys.begin(); i != Citys.end();
             ++i) //战斗及输出
          (*i).Battle();
        for (vector<City>::iterator i = Citys.begin(); i != Citys.end();
             ++i) //奖励生命元
          (*i).Reward(0);
        for (vector<City>::reverse_iterator i = Citys.rbegin();
             i != Citys.rend(); ++i)
          (*i).Reward(1);
        for (vector<City>::iterator i = Citys.begin(); i != Citys.end();
             ++i) //获取city生命元
...
bool Warrior::Minute_5() {
  if (name == lion)
    if (Loyal <= 0) {
      Print_time();
      cout << Command_Belong_to->colour(true) << " lion " << Num
           << " ran away\n";
      return true;
    }
  return false;
...
void Warrior::Minute_30(int Add_life) {
  Print_time();
  cout << Command_Belong_to->colour(true) << ' ' << Knight[name] << ' ' << Num
       << " earned " << Add_life << " elements for his headquarter\n";
...
void Warrior::Be_shoted() {
  Life -= arrow_attack;
  if (Life <= 0) {
    cout << " and killed " << Command_Belong_to->colour(true) << ' '
         << Knight[name] << ' ' << Num;
  }
  cout << endl;
}
...
void Warrior::Use_bomb(Warrior *Wp) {
  Print_time();
  cout << Command_Belong_to->colour(true) << ' ' << Knight[name] << ' ' << Num
       << " used a bomb and killed " << (Wp->Command_Belong_to)->colour(true)
       << ' ' << Knight[Wp->name] << ' ' << Wp->Num << endl;
}
...
void Warrior::Dead() {
  Print_time();
  cout << Command_Belong_to->colour(true) << ' ' << Knight[name] << ' ' << Num
       << " was killed in city " << City_Belong_to->Num() << endl;
}
void Warrior::Fight(Warrior *Wp) {
  ...
  if (Life > 0 and (Wp->Life) > 0) {
    ...
    Print_time();
    cout << Command_Belong_to->colour(true) << ' ' << Knight[name] << ' ' << Num
         << " attacked " << (Wp->Command_Belong_to)->colour(true) << ' '
         << Knight[Wp->name] << ' ' << Wp->Num << " in city "
         << City_Belong_to->Num() << " with " << Life << " elements and force "
         << Attack << endl;
    ...
    if (Wp->Life > 0) {
      if (Wp->name == ninja) {
        ...
        if (Wp->weapon[sword])
          E_swatk = (Wp->weapon[sword])->sw_atk();
        Print_time();
        cout << (Wp->Command_Belong_to)->colour(true) << ' ' << Knight[Wp->name]
             << ' ' << Wp->Num << " fought back against "
             << Command_Belong_to->colour(true) << ' ' << Knight[name] << ' '
             << Num << " in city " << City_Belong_to->Num() << endl;
  ...
  if (name == dragon and Life > 0 and Morale > 0.8) {
    Print_time();
    cout << Command_Belong_to->colour(true) << ' ' << Knight[name] << ' ' << Num
         << " yelled in city " << City_Belong_to->Num() << endl;
  }
  if (Wp_win) {
    Print_time();
    cout << (Wp_win->Command_Belong_to)->colour(true) << ' '
         << Knight[Wp_win->name] << ' ' << Wp_win->Num << " earned "
         << City_Belong_to->Life_() << " elements for his headquarter\n";
