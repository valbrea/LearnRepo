function [valid_throughput, FER, delay] = A3_N3(SNR, slot)
% 输入为SNR_dB和总的仿真时隙数
n = 3;
% 数据包大小
data_bits = 8742;
    ...
    switch type
        case 1
            ...
            if ~isequal(PB_C3, empty) % 如果PB非空 发包
                ...
                % 端节点T4收包
                % (rule 37) 收包后和TB异或，XOR结果非空就递交
                cache_T4 = BPSK(SNR, PB_C3);
                T_receive = XOR(cache_T4, TB_T4);
                % 无论对错都把TB更新为收到的包
                TB_T4 = cache_T4;
                % (rule 35)节点n发包后立刻要清空PB
                PB_C3 = empty;
                % 如果非空就递交上层判断
                % 在 n + 1时刻以后T节点才会收到包
        ...
        case 2
            ...
            if ~isequal(PB_A1, empty) % 如果PB非空 发包
                ...
                H_receive = XOR(cache_H0, TB_H0);
                % 无论对错都把TB更新为收到的包
                TB_H0 = cache_H0;
                % (rule 35)节点1发包后立刻要清空PB
                PB_A1 = empty;
                % 如果非空就递交上层判断
